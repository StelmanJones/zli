...
const ansi = @import("./main.zig").terminal.format;
const StandardStyles = @import("./main.zig").terminal.StandardStyles;
const zli = @import("./main.zig");
const ProgramOptions = zli.ProgramOptions;
pub const args = @import("./cli/args.zig");
pub const parsers = @import("./cli/parsers.zig");
pub const streaming = @import("./cli/streaming.zig");
pub const Theme = @import("./cli/theme.zig").Theme;
test "clap" {
...
pub fn parseParamsComptime(comptime str: []const u8) [countParams(str)]Param(Help) {
    ...
    _ = parseParamsIntoSliceEx(&res, str, &end) catch {
        ...
        @compileError(std.ansi.comptimePrint("error:{}:{}: Failed to parse parameter:\n{s}", .{
            loc.line + 1,
            loc.column + 1,
            loc.source_line,
        }));
...
pub fn version(writer: anytype, theme: Theme, name: []const u8, app_version: std.builtin.Version) !void {
    try ansi.updateStyle(writer, theme.program_name_style, .{});
    try writer.print("{s} ", .{name});
    try ansi.updateStyle(writer, theme.program_version_style, theme.program_name_style);
...
pub fn about(writer: anytype, content: []const u8) !void {
    try writer.print("{s}\n\n", .{content});
}
pub const HelpOptions = struct {
    ...
    program_options: zli.ProgramOptions = .{},
    theme: Theme = .{},
    about: []const u8 = "An App.",
...
/// be passed.
pub fn help(writer: anytype, comptime Id: type, params: []const Param(Id), opt: HelpOptions, theme: Theme, about_content: []const u8) !void {
    try version(writer, theme, opt.program_options.name, opt.program_options.version);
    try writer.print("{s}\n\n", .{about_content});
    const max_spacing = blk: {
        var res: usize = 0;
        for (params) |param| {
            var cs = io.countingWriter(io.null_writer);
            try printParam(cs.writer(), Id, param, theme);
    ...
    for (params) |param| {
        ...
        try printParam(cw.writer(), Id, param, theme);
        ...
        try ansi.updateStyle(writer, theme.description_style, .{});
        ...
        while (it.next()) |raw_line| : (first_line = false) {
            ...
            try ansi.resetStyle(description_writer.underlying_writer);
        }
        try writer.writeAll("\n");
        try ansi.resetStyle(writer);
    }
...
fn printParam(stream: anytype, comptime Id: type, param: Param(Id), theme_opts: Theme) !void {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer {
        _ = gpa.deinit();
    }

    try ansi.updateStyle(stream, theme_opts.param_name_style, .{});
    ...
    if (param.takes_value == .none) {
        try ansi.resetStyle(stream);
        return;
    }
    try ansi.resetStyle(stream);
    try stream.writeAll(" <");
    try ansi.updateStyle(stream, theme_opts.param_value_style, .{});
    var param_type = try std.ascii.allocUpperString(gpa.allocator(), param.id.value());
    try stream.writeAll(param_type);
    gpa.allocator().free(param_type);
    try ansi.updateStyle(stream, StandardStyles.default, theme_opts.param_value_style);
    try stream.writeAll(">");
    if (param.takes_value == .many)
        try stream.writeAll("...");
}
...
pub fn usage(writer: anytype, comptime Id: type, params: []const Param(Id), theme: Theme) !void {
    var cos = io.countingWriter(writer);
    const cs = cos.writer();
    for (params) |param| {
        ...
        if (cos.bytes_written == 0)
            try writer.writeAll("[-");
        try ansi.updateStyle(writer, theme.param_name_style, .{});
    ...
    for (params) |param| {
        ...
        if (param.takes_value != .none) {
            try cs.writeAll(" <");
            try ansi.updateStyle(writer, theme.param_value_style, .{});
